...
def render_report(sub: dict[str, Any], cfg: SubstrateConfig) -> str:
    ...
    if not s["percentiles_valid"]:
        ...
        return "\n".join(out)
    if s["graph_degraded"]:
        out.append("> **Graph degraded**: `load_index` computed without graph inputs and flagged "
                   "`load_index_degraded` on every node. Graph-dependent claims must not be made from this run.\n")

    load_cols = [("load", "load_index"), ("degraded", "load_index_degraded"), ("fan_in", "fan_in"),
                 ("centrality", "centrality"), ("fan_out", "fan_out"), ("loc", "size_loc"), ("cochange", "cochange_degree")]
    out.append(f"## 1. Highest `load_index` (top {k})\n")
    out.append(_table(_rows(nodes, "load_index", k), load_cols))

    change_cols = [("change", "change_pressure_index"), ("churn", "churn_lines"), ("commits", "commit_count"),
    ...
    bug_cols = [("bug", "bug_pressure_index"), ("fixes", "fix_count"), ("reverts", "revert_count"),
                ("commits", "commit_count"), ("reinforcement", "reinforcement_index")]
    out.append(f"## 3. Highest `bug_pressure_index` (top {k})\n")
    out.append(_table(_rows(nodes, "bug_pressure_index", k), bug_cols))

    out.append(f"## 4. High load, low reinforcement — `load_index ≥ {p} ∧ reinforcement_index ≤ {q}` (top {k} by load)\n")
    out.append(_table(_rows(nodes, "load_index", k, lambda i, m: (i.get("load_index") or 0) >= p and (i.get("reinforcement_index") or 0) <= q),
                      load_cols[:4] + [("reinforcement", "reinforcement_index"), ("bug", "bug_pressure_index")]))

    out.append(f"## 5. Old, load-bearing, untouched — `neglect_index ≥ {p} ∧ load_index ≥ {q}` (top {k} by neglect)\n")
    out.append(_table(_rows(nodes, "neglect_index", k, lambda i, m: (i.get("neglect_index") or 0) >= p and (i.get("load_index") or 0) >= q),
                      [("neglect", "neglect_index"), ("load", "load_index"), ("age_d", "age_days"),
                       ("last_touched_d", "last_touched_days"), ("blame_age_d", "blame_age_median"), ("fan_in", "fan_in")]))

    cav = sub.get("caveats", {})
    out.append("## Caveats\n")
    if cav.get("orphan_nodes"):
        out.append(f"- **Orphan nodes ({len(cav['orphan_nodes'])})** — files at the rev with no history (miner defect signal, §5): "
                   + ", ".join(f"`{p}`" for p in cav["orphan_nodes"][:20]) + ("…" if len(cav["orphan_nodes"]) > 20 else ""))
    if cav.get("unresolved_import_samples"):
        out.append(f"- **Unresolved in-repo imports** (first {len(cav['unresolved_import_samples'])} of {s['unresolved_imports']}): "
                   + ", ".join(f"`{x['from']}` → `{x['specifier']}`" for x in cav["unresolved_import_samples"][:10]))
