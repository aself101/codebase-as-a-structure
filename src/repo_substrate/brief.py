...
def facts(skeleton: dict[str, Any], substrate: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    # D-079: rooms in a closed import cycle — PageRank keeps the rank that enters one
    in_cycle: set[str] = set()
    if substrate is not None:
        for c in _closed_cycles([n["id"] for n in substrate.get("nodes", [])], [(ed["from"], ed["to"]) for ed in substrate.get("edges") or []]):
            in_cycle |= c
    ...
    for e in feats.values():
        ...
        # D-079 (the re-seat after D-078): a row reading centrality counts its rooms in a closed import cycle
        if _reads_centrality(e["predicate"]):
            e["cycle_rooms"] = sum(1 for r in e["rooms"] if r in in_cycle)
    ...
    doc = {
        ...
        "kind_conventions": {k: v for k, v in ((((substrate or {}).get("repo") or {}).get("effective_config") or {})).items() if k in ("config_file_regex", "migration_dir_regex", "placeholder_content_regex", "example_dir_regex")},
        # D-078: the directories the example convention matched, with the relation it stands in for
        "example_dirs": list((((substrate or {}).get("summary") or {}).get("example_dirs")) or []),
        # D-078 (both reviews): the blends rank against the substrate's population, not the page's rooms
        "rank_population": _rank_population(substrate, skeleton),
        ...
        "centrality_illustration": _with_cycle(_centrality_illustration(feats, nodes, population_ids), in_cycle),
...
KIND_CONVENTION_WORDS = {
    "example": "any file under a directory named `examples`, `samples`, `demos`, `cookbook` or `playground`, or the same with a leading underscore — the declared stand-in for a consumer of the repository's product (D-078)",
...
def _example_dirs_text(facts_doc: dict[str, Any]) -> str:
    """D-078 (the Foucault review): the example kind is a word list standing in for a relation — a consumer
    of the product imports into it and is not imported back; the page names each directory the convention
    removed with that relation counted, so the proxy can be checked where it is used."""
    if "example" not in (facts_doc.get("excluded_kinds") or []):
        return ""
    ds = facts_doc.get("example_dirs") or []
    if not ds:
        return ""
    return "; example directories: " + "; ".join(
        f"{d['dir']} ({d['files']} file{'s' if d['files'] != 1 else ''}; {d['imports_out']} import{'s' if d['imports_out'] != 1 else ''} from it into the rest of the repository, {d['imported_back']} into it from the rest)"
        for d in ds
    )


def _rank_population(substrate: dict[str, Any] | None, skeleton: dict[str, Any]) -> dict[str, int] | None:
    """D-078: the files every blend input is ranked among (the substrate's population: non-test, with
    history — assemble.py §6.1), and how many of them are of a kind the ruleset does not count as rooms.
    The page's pNN cutoffs rank its rooms; the blends' inputs were ranked here, before the ruleset chose."""
    if not substrate:
        return None
    excluded = set((skeleton.get("summary") or {}).get("excluded_kinds") or [])
    files = [n for n in substrate.get("nodes", []) if not n["metrics"].get("is_test") and not n["metrics"].get("history_missing")]
    return {"files": len(files), "excluded_kind": sum(1 for n in files if n["metrics"].get("file_kind", "source") in excluded)}


def _example_importers_text(imp: dict[str, Any]) -> str:
    """D-078: example files among a row's importers — excluded from the rooms, still importers (mcp: 20 of
    src/index.ts's 21)."""
    k = imp.get("example_files") or 0
    return f", {k} of them example files" if k else ""
...
def _closed_cycles(ids: list[str], edges: list[tuple[str, str]]) -> list[set[str]]:
    """D-079: the strongly connected sets of two or more nodes that no edge leaves — PageRank's rank sinks:
    rank that enters one does not leave but by the teleport term (graph.py). Iterative Tarjan."""
    g: dict[str, list[str]] = {}
    for a, b in edges:
    ...
    comp = {v: k for k, c in enumerate(comps) for v in c}
    return [c for c in comps if len(c) > 1 and all(comp.get(b) == comp[a] for a in c for b in g.get(a, []))]


def _reads_centrality(predicate: str) -> bool:
    """D-079: whether a predicate reads PageRank — directly, or through a blend whose declared inputs hold
    it (load_index). `_signals_read` expands centrality to its grounding input (fan_in), which is right for
    the record column and wrong here."""
    from .config import ALLOWED_INPUTS
    from .mapper.ruleset import parse_predicate

    return any(t.signal == "centrality" or "centrality" in ALLOWED_INPUTS.get(t.signal, ()) for t in parse_predicate(str(predicate or "")))


def _cycle_rooms_text(k: int, count: int) -> str:
    """D-079: the rows reading centrality count the rooms a closed cycle holds."""
    if not k:
        return ""
    return f" ({k} of these {count} {'sits' if k == 1 else 'sit'} in a closed import cycle — rooms that import one another and nothing outside, whose PageRank the cycle keeps)"


def _with_cycle(ill: dict[str, Any] | None, in_cycle: set[str]) -> dict[str, Any] | None:
    if ill is None:
        return None
    return {**ill, "hub_in_cycle": ill["hub_room"] in in_cycle}
...
def _centrality_illustration_text(facts_doc: dict[str, Any]) -> str:
    ...
    if not ill:
        return ""
    # D-079: "well-placed" graded the importer with no field behind it, and on mcp-secure-server the importer
    # sat in the hub's own closed cycle; the example says what the sheet holds, and names the cycle
    cyc = f"; {ill['hub_room']} sits in a closed import cycle, which keeps the rank that enters it" if ill.get("hub_in_cycle") else ""
    return f", so a room with {ill['hub_fan_in']} importer{'s' if ill['hub_fan_in'] != 1 else ''} can outrank one with {ill['other_fan_in']} (here {ill['hub_room']} at {ill['hub_fan_in']} is a {ill['feature']} and {ill['other_room']} at {ill['other_fan_in']} is not{cyc})"
...
def _index_legend_lines(facts_doc: dict[str, Any]) -> str:
    ...
    for idx, w in (facts_doc.get("index_weights") or {}).items():
        ...
        rp = facts_doc.get("rank_population")
        among = (
            f"each input a rank among this repository's {rp['files']} non-test files"
            + (f", {rp['excluded_kind']} of them of a kind the ruleset does not count as rooms" if rp.get("excluded_kind") else "")
            if rp
            else "each input a rank in this repository's own population"
        )  # D-078: the blends' population is the substrate's, not the page's rooms
        out += f"- **{idx}** — a blend the ruleset reads as one signal, {among}: {terms} (the tuned weights, D-009; the substrate's effective config){zero_text}." + NL
    return out
...
def _importer_files(rooms: list[str], nodes: dict[str, Any], substrate: dict[str, Any]) -> dict[str, Any]:
    ...
    out: dict[str, Any] = {
        ...
        # D-078; D-079 (the skimmer): outside the test convention, so the example and test counts are disjoint
        "example_files": sum(1 for f in files if ((nodes.get(f) or {}).get("metrics") or {}).get("is_example") and not is_test(f)),
    }
    ...
    for f in sorted(facts_doc["features"], key=lambda x: (bool(x["decorative"]),)):
        ...
        if f.get("importers") and f["importers"]["total"]:
            ...
            if "files" in imp:
                ...
                nf = imp["files"]
                what += f" — imported by {nf} file{'s' if nf != 1 else ''}, {imp['test_files']} ({100.0 * imp['test_files'] / nf if nf else 0:.0f}%) of them test files{_example_importers_text(imp)} ({imp['total']} import edges; a file importing several of these rooms is one file here)"
                if imp.get("top"):
                    t = imp["top"]
        ...
        tie += _cycle_rooms_text(f.get("cycle_rooms") or 0, f["count"])  # D-079
