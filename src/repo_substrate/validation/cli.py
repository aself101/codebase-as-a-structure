...
def _blind_seal(path: Path) -> dict[str, str | None]:
    ...
    return {
        ...
        "first_commit": git("log", "--diff-filter=A", "--format=%H", "--", str(path)).splitlines()[-1:] and
                        git("log", "--diff-filter=A", "--format=%H", "--", str(path)).splitlines()[-1] or None,
        "first_commit_time": git("log", "--diff-filter=A", "--format=%cI", "--", str(path)).splitlines()[-1:] and
                             git("log", "--diff-filter=A", "--format=%cI", "--", str(path)).splitlines()[-1] or None,
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    run.add_argument("--repo", "--test-repo", dest="test_repos", type=Path, action="append", default=[],
                     help="TEST-role repo (D-009): only these can confer `validated`")
    run.add_argument("--tuning-repo", dest="tuning_repos", type=Path, action="append", default=[],
                     help="TUNING-role repo: scored and reported in-sample, never counted toward the verdict")
    run.add_argument("--tuned-config-commit", default=None,
    ...
    if args.cmd == "tune":
        ...
        (args.out / "tuning.json").write_text(json.dumps(result, indent=1, sort_keys=True, allow_nan=False) + "\n", encoding="utf-8")
    ...
    if not args.test_repos:
        ap.error("at least one --repo/--test-repo is required (D-009: the verdict comes from test-role repos)")
    ...
    if tuned_commit is None and args.config is not None:
        ...
        proc = subprocess.run(["git", "log", "-1", "--format=%H", "--", str(args.config)],
    ...
    for repo, role in repos:
        ...
        holdouts.append(h)
        asserted.append(a)
        refs.append({"name": h.name, "head_sha": h.head_sha, "path": str(repo), "role": role,
    ...
    doc = build_validation(holdouts, asserted, vcfg, cache.fingerprint, refs, tuned_commit,
                           cache.effective_config(), cache.attestations)
    args.out.mkdir(parents=True, exist_ok=True)
    (args.out / "validation.json").write_text(json.dumps(doc, indent=1, sort_keys=True, ensure_ascii=False, allow_nan=False) + "\n", encoding="utf-8")
