...
- **The hatch is audited, because it is the obvious deadline bypass** (tribunal A3/A6). A `decorative: true` rule must carry a `decorative_reason` string naming the ungrounded signal(s) and why the feature is kept; C3 refuses a decorative rule with no reason, and (D-030) one whose reason names none of the predicate's signals. A feature *name* that carries a consequence word (`NAME_CONSEQUENCE_WORDS` in the loader) must declare `name_implies_consequence` with a `position_name`; the list is the audit surface. Decorative features are excluded from diagnosis, not from measurement: the skeleton diff reports their churn beside the verdict (D-030). The skeleton records `summary.decorative_count` and `summary.decorative_features[]`, and the C6 render and C5 brief must surface that count where a reader sees it — a building with eleven decorative features and two grounded ones announces itself. This does not make the hatch unabusable; it makes abuse visible in the artifact rather than in a config file. *(Added 2026-09-04.)*
...
- **The population is one (D-067, 2026-09-13).** A room is a substrate node outside the test convention with computed indices and *not of a kind the ruleset excludes* (`[ruleset] exclude_kinds`, drawn from the substrate's `file_kind`: `config`, `migration`, `placeholder`, and from D-078 `example` — G1 flags by declared convention, repo-substrate spec §5; the substrate still ranks every kind, so the blends' inputs are ranked among files the ruleset does not count, and the page says so). Every pNN ranks the ruleset's rooms (`engine._resolve_thresholds`), so the exclusion changes the ranks, and an overlay must exclude the same kinds as its base or the engine refuses it — two profiles over two populations would be two buildings. The skeleton records `excluded_kinds` and `excluded_by_kind`; the brief's population rule prints both. The choice is the ruleset's, not the substrate's: the substrate records the kind and ranks nothing by it; the shipped profiles (maintainability 0.3.0, onboarding 0.3.0) exclude all three, each stating why in its header. What forced it: on mcp-secure-server fourteen test-runner and lint configs and two `export {}` barrels were the whole of `flooded_basement`'s fan_in-0 case (D-064, D-066); on the registry eleven migrations were the largest parent of the dark rooms (D-066's skimmer) — files whose correct state is untouched, marked long-untouched.
...
6. **Absolute-era strata for the age geometry (D-026).** Age bands are quantiles of `age_days`; on a growing population they re-rank wholesale between frames (K study: untouched strata movement p90 0.13–0.43 under age geometry against ≤ 0.13 under layer). If the age geometry is to hold a floor still, its bands should be absolute eras — D-019's calibration argument applied to strata — at the cost that a young repository fills two or three bands. Not built; the time-lapse uses layer geometry where stability matters.
...
7. **Per-package calibration (monorepo pooling).** *Open.* Assigned to the mapper by architect-brief spec §5 Q5 ("per-component calibration is a mapper question") and carried on every page since D-053; recorded here at D-054, when the fifteenth seating found the page citing this list for a question it did not hold. Every pNN ranks one repository's rooms as one population (system spec §5.3) across every `package.json` scope the population spans, pooled; the brief states the count and the rooms per scope so the pooling has a size (eslint: 435 of 473 rooms in the root scope, five scopes holding the other 38; mcp-secure-server: 66 of 202 in the root, fourteen scopes under `cookbook/` holding 136, the largest 17). Whether a scope of a dozen rooms should be ranked against a scope of four hundred is unanswered: a per-scope population would fall under the population floor (30 rooms, D-034) on every scope but the root on both, and a mixed rule (pool below the floor, split above it) is a calibration mode §5.3 does not have. *D-077 (`scope-calibration-spec.md`, two reviews): measured — immaterial on registry and eslint, moderate on typeorm, concentrated on mcp-secure-server (the library, 65 of 187 rooms, holds 17 of 19 foundation and all 27 lit rooms). The page states the pooling as composition (brief 0.42.0). The mixed rule is demoted (a floor cliff and a mode that changes across time-lapse frames) and per-scope substrate ranks dropped (at most two rooms a feature). The question is reframed as room definition: whether a scope with a declared example role is a room — open with Alex.*
8. **Corpus-relative fields under D-004 Q3 (D-059).** *Decided in form, open in content.* A corpus-relative claim is a field with a corpus denominator — a position in the corpus's distribution (system spec §5.3, D-059 revision) — never a verdict. Open: which fields get a corpus column first (per-feature share of rooms is the obvious one; the most-marked count and the co-location share are candidates), what n the corpus must reach before a decile is printed (the population floor's analogue, D-034), and whether a corpus position on a decorative feature is printed at all.
