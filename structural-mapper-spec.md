...
3. **Stability budget — skeleton level.** The *signal-level* budget is resolved (`validation-spec.md` §2.4.1, D-004). What remains is the feature-level question: how many features may appear, disappear, or change node between adjacent commits before the time-lapse reads as jitter. An M2 question; proposed starting point is a per-feature-type churn rate measured on the reference repos' last 50 commits.
