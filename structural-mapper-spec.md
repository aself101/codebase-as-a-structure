...
3. **Stability budget — skeleton level.** The *signal-level* budget is resolved (`validation-spec.md` §2.4.1, D-004). The *instrument* for the feature-level question now exists: `substrate skeleton-diff` reports per-feature added/removed/Jaccard over common nodes, `feature_churn`, and `strata_moved` (D-017). The budget number is still to be set from data: measure churn between HEAD and HEAD−K on the reference repos and pin a ceiling. First reading on `uluops-registry-api` (K = 5 commits) is recorded in D-017.
