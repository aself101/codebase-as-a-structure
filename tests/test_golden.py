...
def test_rename_onto_a_reused_name(make_repo, small_cfg, tmp_path):
    ...
    r.commit("feat: x")
    r.git("mv", "x.js", "y.js")
    r.commit("refactor: x -> y")
    r.write("z.js", "2\n")
    r.commit("feat: z")
    r.git("mv", "z.js", "x.js")
    r.commit("refactor: z -> x")
    r.write("x.js", "3\n")
    r.commit("fix: new x")
...
def test_config_validate_error_paths():
    with pytest.raises(ValueError, match="sum to"):
        SubstrateConfig(
            weights=IndexWeights(load_index={"fan_in_nonzero": 0.6, "centrality": 0.6})
        ).validate()
    with pytest.raises(ValueError, match="unknown inputs"):
        SubstrateConfig(
            weights=IndexWeights(load_index={"fan_in_nonzero": 0.5, "bogus": 0.5})
        ).validate()
