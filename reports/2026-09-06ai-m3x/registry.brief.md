...
*Rendered from the facts sheet by code; no model wrote any of it. Every cell is a field, and every fixed text on the page has a test that asserts it against the sheet it is rendered from (`tests/test_brief.py`); the tests run on a fixture, not on this page. Profile maintainability 0.2.10 + onboarding 0.2.6, geometry age, skeleton `6b3cd65a8056…`, facts `bc24309f3cfa…`. As of 2026-08-31, commit `f7414cc7bbec`. Calibration: in-repo, self-relative (system spec §5.3); one frame — stability is the `substrate timelapse` run under gate `179d8acb7b0c`, not this page. Brief 0.30.0.*
...
| recently-touched room (clock) | lit_room | maintainability | 40 (14 tied at the cutoff) | src 40 | none holds a third | no identity or containment | `last_touched_days <= p10` — rooms at or below this repository's p10 on last_touched_days (here 9.1099 days): 40 of 267, 15.0% |
...
- facts_hash: `bc24309f3cfa79b0c7d2b114a916d26b1805c4323fba8eda60cd15f54b032ba3`
- generator: `code`
