...
| foundation | maintainability | high-load node (import graph and size) | 21 | cookbook 7, src 14 | none holds a third | ⊃ toothpick_wing (18 of these rooms outside it); = foundation (same predicate, two profiles) | `load_index >= p90` |
...
| ◌ toothpick_wing | maintainability | unreinforced high-load node with high edit pressure (import graph and test graph and edit record and size) | 3 | src 3 | too few rooms to place (3) | ⊂ crack (18 crack rooms outside this set); ⊂ foundation (18 foundation rooms outside this set); ⊂ lit_room (18 lit_room rooms outside this set); ⊂ foundation (18 foundation rooms outside this set) | decorative — bug_pressure_index is unvalidated on the pre-registered test set (D-015): its tuned weights assign zero to fix history and it does not beat busyness by the margin. The fragility half of this feature has no forecast to stand on; it renders as decorative, counted, and excluded from diagnostic claims until a reference repo validates the index. |
...
| foundation | onboarding | high-load node (import graph and size) | 21 | cookbook 7, src 14 | none holds a third | = foundation (same predicate, two profiles); ⊃ toothpick_wing (18 of these rooms outside it) | `load_index >= p90` |
...
*1 room carries the most diagnostic marks (7, one per feature per profile); all are listed.*

| room | marks | diagnostic features that mark it |
|---|---|---|
| cookbook/filesystem-server/src/utils/path-validator.ts | 7 | dark_room, flooded_basement, foundation, hub, leaf_utility, scaffolding |
| cookbook/filesystem-server/src/utils/index.ts | 6 | corridor, dark_room, flooded_basement, foundation, hub |
| src/security/utils/error-sanitizer.ts | 6 | corridor, foundation, hub, lit_room, scaffolding |
| cookbook/api-wrapper-server/src/utils/index.ts | 5 | corridor, dark_room, flooded_basement, hub, scaffolding |
| cookbook/transaction-server/src/utils/index.ts | 5 | corridor, foundation, hub, scaffolding |
...
- facts_hash: `58b59cff62dae1f6cd66e6a683bb1ede9c64e22da9e5f52a95e698b626885b32`
- generator: `code`
