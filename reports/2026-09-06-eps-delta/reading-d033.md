...
92 (signal, repo) readings reached the stability test: 44 own (class check — zero by construction, D-033 addendum; 0 read above 0.000), 46 coupled and non-degenerate (the bars are placed over these).

| operand | threshold | reference-set median | p90 | max | fires on | verdict under the D-026 rule (letter) |
|---|---|---|---|---|---|---|
| eps over median \|Δ\| | 0.010 | 0.0000 | 0.0043 | 0.0162 | 3 of 46 | above p90 — cannot fail on this set |
| delta over the tail operand | 0.050 | 0.0011 | 0.0203 | 0.1195 | 2 of 46 | above p90 — cannot fail on this set |

| signal | ripple | repo | median \|Δ\| | p95 | max | operand | n | status |
|---|---|---|---|---|---|---|---|---|
| `neglect_index` | coupled | mcp-secure-server | 0.0080 | 0.1195 | 0.2394 | p95 | 179 | untested **unstable** |
| `neglect_index` | coupled | uluops-registry-api | 0.0162 | 0.0645 | 0.2278 | p95 | 222 | untested **unstable** |
| `recent_commit_share` | coupled | uluops-registry-api | 0.0090 | 0.0315 | 0.6329 | p95 | 222 | asserted |
| `neglect_index` | coupled | typeorm | 0.0103 | 0.0307 | 0.1014 | p95 | 529 | untested **unstable** |
| `centrality` | coupled | mcp-secure-server | 0.0028 | 0.0251 | 0.0363 | p95 | 179 | asserted |
| `centrality` | coupled | uluops-registry-api | 0.0135 | 0.0203 | 0.2095 | p95 | 222 | asserted **unstable** |
...
| `author_count` | own | uluops-registry-api | 0.0000 | 0.0000 | 0.0000 | max | 222 | asserted *degenerate* |
...
| `is_package_entry` | coupled | uluops-registry-api | 0.0000 | 0.0000 | 0.0000 | p95 | 222 | asserted *degenerate* |
...
| `recent_commit_share` | coupled | mcp-secure-server | 0.0000 | 0.0000 | 0.0000 | p95 | 179 | asserted *degenerate* |
...
| `revert_count` | own | mcp-secure-server | 0.0000 | 0.0000 | 0.0000 | max | 179 | untested *degenerate* |
| `revert_count` | own | uluops-registry-api | 0.0000 | 0.0000 | 0.0000 | max | 222 | untested *degenerate* |
| `revert_count` | own | eslint | 0.0000 | 0.0000 | 0.0000 | max | 455 | untested *degenerate* |
