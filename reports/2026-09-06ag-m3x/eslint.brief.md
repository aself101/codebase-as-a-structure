...
*Rendered from the facts sheet by code; no model wrote any of it. Every cell is a field, and every fixed text on the page has a test that asserts it against the sheet it is rendered from (`tests/test_brief.py`); the tests run on a fixture, not on this page. Profile maintainability 0.2.9 + onboarding 0.2.5, geometry age, skeleton `cf0c90021418…`, facts `064ca974b9c4…`. As of 2026-09-04, commit `3f20a57c6293`. Calibration: in-repo, self-relative (system spec §5.3), 6 package scopes pooled — per-package calibration is an open question of the mapper (architect-brief spec §5, mapper §7 Q7); one frame — stability is the `substrate timelapse` run under gate `179d8acb7b0c`, not this page. Brief 0.28.0.*
...
| high-centrality node (import graph) | hub | maintainability | 50 | conf 2, docs 1, lib 46, packages 1 | none holds a third | ⊃ corridor (by its predicate; 28 of these rooms outside it) | `centrality >= p90` — rooms at or above this repository's p90 on centrality (here 0.0012): 50 of 473, 10.6%; 3 at the cutoff value — importers of these rooms: 696, 358 (51%) from test files |
...
| high-centrality junction with fan-out at or above the median (import graph) | corridor | onboarding | 22 | lib 21, packages 1 | none holds a third | ⊂ hub (by its predicate; 28 hub rooms outside this set) | `centrality >= p90 and fan_out >= p50` — importers of these rooms: 583, 328 (56%) from test files |
...
| imported leaf with fan-in at or above the upper quartile (import graph) | leaf_utility | onboarding | 148 | conf 2, docs 2, lib 139, messages 1, packages 2, tools 2 | lib/rules 98 / 293 | no identity or containment | `fan_out == 0 and fan_in >= p75` — importers of these rooms: 359, 146 (41%) from test files |
...
- facts_hash: `064ca974b9c4feffb055dcbe5f6e9de237a119b73bd4498d9171013a4a18cca2`
- generator: `code`
