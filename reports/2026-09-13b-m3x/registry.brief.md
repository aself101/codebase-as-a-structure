...
*Rendered from the facts sheet by code; no model wrote any of it. Every cell is a field, and every fixed text on the page has a test that asserts it against the sheet it is rendered from (`tests/test_brief.py`); the tests run on a fixture, not on this page. Profile maintainability 0.2.11 + onboarding 0.2.7, geometry age, skeleton `32a92e99ba2e…`, facts `0a551f72d29a…`. As of 2026-09-13, commit `f4f6ebd23eb4`. Calibration: in-repo, self-relative (system spec §5.3); one frame — stability is the `substrate timelapse` run under gate `661f93333b65`, not this page. Brief 0.32.0.*

## Register

**A position names where a room sits in a record — the import graph, the clock, the test graph, the edit record, size — and is not a claim about the room's condition (D-004 Q3). Every pNN ranks this repository's own 267 rooms, so a `>= p90` row holds a tenth of them — or more where rooms tie at the cutoff, and the rooms column says how many are tied; a single-rank row states its share, and every ranked term states the value its rank resolved to here — and no count on this page compares across repositories. A room is a source file outside the test convention with computed signals: 267 of the 456 files with a source extension the substrate reads (manifests, documents and the rest of the tree are not counted); the 189 test files are nodes of the import graph and not rooms.**

*267 rooms in 3 wings ((root) 7 · scripts 30 · src 230); 352 diagnostic marks across all profiles (280 in the base profile), one mark per feature per room; identical pairs of diagnostic features mark 27 rooms twice (27 under one predicate in two profiles, 0 where two predicates draw one set because a conjunct excludes nothing, 0 under both); the diagnostic features name 10 distinct sets of rooms; 27 excluded marks ◌ (crack 27, toothpick_wing 0); 88 rooms carry two or more distinct diagnostic sets; gate `661f93333b65`, 7 of 8 signals asserted, none validated — asserted is a description that held under the stability budget and the corroboration its grounding class requires (validation spec §2.4), validated a forecast confirmed by a temporal holdout (validation spec §3).*
...
|---|---|---|---|---|---|---|---|
| long-untouched room (clock) | dark_room | maintainability | 27 | scripts 1, src 26 | src/db/migrations 11 / 55 | ⊃ flooded_basement (by its predicate; 1 of these rooms outside it) | `last_touched_days >= p90` — rooms at or above this repository's p90 on last_touched_days (here 189.971 days): 27 of 267, 10.1%; the next value below the cutoff holds 1 room, 0.06752 days under it |
| long-untouched room above the load floor (import graph and clock and size) | flooded_basement | maintainability | 26 | scripts 1, src 25 | src/db/migrations 11 / 55 | ⊂ dark_room (by its predicate; 1 dark_room room outside this set) | `last_touched_days >= p90 and load_index >= 0.10` — here p90 on last_touched_days is 189.971 days — importers of these rooms: 73, 7 (10%) from test files — caveat: reads a floor on the load blend, not an importer (D-048): a room that nothing imports can clear the floor on inverse fan-out and size alone (12 of 26 here have no importer) |
| high-load node (import graph and size) | foundation | maintainability + onboarding (one predicate, two profiles) | 27 | src 27 | none holds a third | no identity or containment | `load_index >= p90` — rooms at or above this repository's p90 on load_index (here 0.78956): 27 of 267, 10.1%; the next value below the cutoff holds 1 room, 0.00016 under it — importers of these rooms: 535, 138 (26%) from test files |
| high-centrality node (import graph) | hub | maintainability | 27 | src 27 | none holds a third | ⊃ corridor (by its predicate; 14 of these rooms outside it) | `centrality >= p90` — rooms at or above this repository's p90 on centrality (here 0.00474): 27 of 267, 10.1%; the next value below the cutoff holds 1 room, 4e-05 under it — importers of these rooms: 488, 127 (26%) from test files |
| recently-touched room (clock) | lit_room | maintainability | 40 (14 tied at the cutoff) | src 40 | none holds a third | no identity or containment | `last_touched_days <= p10` — rooms at or below this repository's p10 on last_touched_days (here 21.8106 days): 40 of 267, 15.0%; the next value above the cutoff holds 2 rooms, 0.9957 days over it |
...
| high-centrality junction with fan-out at or above the median (import graph) | corridor | onboarding | 13 | src 13 | none holds a third | ⊂ hub (by its predicate; 14 hub rooms outside this set); ⊂ scaffolding (no raw signal in common, one edge set read twice; 120 scaffolding rooms outside this set) | `centrality >= p90 and fan_out >= p50` — here p90 on centrality is 0.00474, p50 on fan_out is 1 — importers of these rooms: 262, 58 (22%) from test files |
| import-graph root with fan-out at or above the upper quartile (import graph) | import_root | onboarding | 11 | scripts 8, src 3 | scripts (as parent, not the wing) 8 / 27 | no identity or containment | `fan_in == 0 and fan_out >= p75` — here p75 on fan_out is 4 — caveat: reads no fan-in, not entrance (D-029): a package entry that anything imports — a room or a test file — cannot qualify, and one that nothing imports can (0 of 11 here are declared package entries) |
| imported leaf with fan-in at or above the upper quartile (import graph) | leaf_utility | onboarding | 20 | src 20 | src/utils 9 / 22 | no identity or containment | `fan_out == 0 and fan_in >= p75` — here p75 on fan_in is 4 — importers of these rooms: 270, 82 (30%) from test files |
...
|---|---|---|---|
| src/utils/async-handler.ts | 36 | 16 (1) | dark_room, flooded_basement, foundation (2 profiles), hub, leaf_utility, scaffolding |
...
- facts_hash: `0a551f72d29aeaefd84e376c4b55ee9b30b1edda6f47534922034254ac663e34`
- generator: `code`
