...
### Tier 1 (required)
`size_loc`, `age_days` (rename-followed first-touch; **fractional** days since D-022, so only files born in one commit tie), `last_touched_days` (fractional likewise), `commit_count`, `churn_lines` (Σ added+deleted), `fix_count` (commits where `type ∈ {fix, revert}`), `revert_count`, `author_count`, `fan_in`, `fan_out`, `is_test`, `has_sibling_test`, `package` (nearest ancestor directory holding a `package.json`, `\"\"` at the root; not a signal) and `is_package_entry` (the file a `package.json` names as `main`, `module`, `browser`, `types`, `bin`, or an `exports` target, resolved to source by a declared heuristic — a G1 flag, D-029), `file_kind` with its G1 flags `is_config`, `is_migration`, `is_placeholder` (D-067, substrate 0.7.0: a declared convention each — a tool-configuration basename at the package root, a `migration(s)/` segment above the file, an empty export once comments are stripped; regexes in `SubstrateConfig`, fingerprinted; precedence config > migration > placeholder > example;) and `is_example` (D-078, substrate 0.11.0: a directory named examples/samples/demos/cookbook/playground, an optional leading underscore — the declared proxy for a consumer of the product, with the relation recorded per matched directory in `summary.example_dirs`: files, imports out, imports back; `summary.file_kinds` counts them over the non-test nodes; a *ruleset* chooses whether a kind is a room, the substrate only records it), `introduced_idx` (timeline position of introducing commit), and — promoted from Tier 2 on 2026-09-04 (D-004) because the `asserted` bar needs them as cross-modal counterparts — `cochange_degree` and `blame_age_median`.
...
- **`fan_in_alt`, `fan_out_alt`** — in-repo import edges found by an independent regex scanner with its own resolver (`altdeps.py`): `import … from`, `export … from`, `require()`, and `import()` specifiers; relative paths resolved by extension and `index` probing; `tsconfig` `baseUrl`/`paths` aliases honored; bare specifiers ignored. Same edge contract as §8 (self-loops dropped, duplicates collapsed). It shares no code with the primary extractor, which is the point: where they disagree, the disagreement is the measurement of extractor fidelity (open question #8). `null` for non-JS/TS nodes. *Correction (D-065, 2026-09-13):* the two instruments share one thing — `deps.load_tsconfig`, the JSONC loader both read their aliases from — and a fault there is invisible to the G2 check: through substrate 0.4.1 the loader's comment stripper read the `/*` in `"@/*": ["./src/*"]` as a comment opener, both instruments ran alias-less on mcp-secure-server, and τ-b read 1.00 while 34 alias imports went unresolved. The loader is now string-aware (`strip_jsonc`); the shared dependency stays, stated.
...
- Dependency extraction behind a `DependencyExtractor` protocol; v0 ships JS/TS (shell out to `madge`/`dependency-cruiser` or parse imports with `tree-sitter`). The protocol's edge contract is pinned so `fan_in`/`fan_out` are well-defined:
  ...
  - **Only resolved, in-repo targets become edges**, and the two kinds of non-edge are counted **separately** — this distinction is load-bearing for §6.3:
    - **External imports** (a package / out-of-repo path the extractor *correctly* resolved as not-in-this-repo, e.g. `import "react"`) → counted in `summary.external_imports`. These are *successful* resolutions to an out-of-repo target; they are **not** a quality problem and are **excluded** from the resolution rate. *Addendum (D-066, 2026-09-13):* a bare specifier equal to the `name` of a `package.json` in the tree is **not** external — it names this repository (a cookbook example under a `file:../..` manifest importing the library it demonstrates; a codemod fixture importing `"typeorm"` inside typeorm). Substrate 0.6.0 resolves it to that package's runtime entry (`inventory.package_facts` → `entry_by_name`: the first of `main`, `module`, `browser` that resolves to a node, by the same built→source heuristic `is_package_entry` uses) and counts it in `summary.package_name_imports`; a *subpath* of that name (`typeorm/metadata/X`) addresses the package's built layout, which the tree does not hold, and is counted **unresolved** (in-repo-shaped), never external. Both instruments read one map (§5 correction, D-065), stated. On the four reference repositories the count was mcp-secure-server 19, typeorm 226 (209 of them codemod fixtures), eslint 15, uluops-registry-api 1 — every one external through 0.5.0.
...
8. **Extractor fidelity (mis-resolution).** `graph_resolution_rate` (§6.3) catches *non*-resolution but not confident *mis*-resolution (an import resolved to the wrong in-repo file via path-alias / re-export). How is the extractor's mapping spot-validated, and on which repos, before `load_index` rankings are trusted? Carry until the JS/TS extractor is built and run against alias-heavy code. *Addendum (D-065, 2026-09-13):* the first alias failure the seatings reached was not mis-resolution but non-resolution with a wrong cause on the page — the substrate's own tsconfig loader (see §5, second-instrument correction). Confident mis-resolution remains unmeasured; the reference corpus has one alias-bearing repository (mcp-secure-server, two `paths` patterns), and after 0.5.0 its unresolved count is stated in D-065. *Addendum (D-066):* the second control on the same page found the next classification below the page — the repository's own package name counted as external on all four reference repositories (§8 addendum). Two consecutive substrate findings in the import classification; the next unpointed control on a multi-scope repository will reach for the same layer, and the bundler-alias case (D-065's breaks-if) is still open.
