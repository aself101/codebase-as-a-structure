...
A `kind: descriptive` signal earns `asserted` only by passing **both** gates below on **≥ `M_asserted` reference repos** (config, default 3 since D-033 — a majority of the four-repository set; 2 until then). Either alone is insufficient: stability alone certifies a constant (`load_index ≡ 0.5` never moves), and a correctness check alone certifies noise that happened to correlate once.

#### 2.4.1 Stability budget (robustness)

Recompute the substrate at HEAD with the last **K** commits removed (config `stability_perturbation_k`, default `25` since D-031 — the K at which the skeleton budget was pinned, D-026; the June placeholder was `5`, at which D-024 showed nothing fires) and measure, per node, each signal's percentile movement against the unperturbed run.

Pass iff `median(|Δ|) ≤ stability_eps` (default `0.01` since D-033; `0.05` until then) **and** `tail(|Δ|) ≤ stability_delta` (default `0.05` since D-033; `0.15` until then) over the **untouched** population: nodes present in both runs and **not edited by any of the K removed commits** (D-008). **The comparison value (D-033):** an index or a flag compares as itself; every other signal compares on its percentile **ranked over the untouched population in both runs**, so a touched file's own movement cannot re-rank the untouched ones — before D-033 the percentiles came from each run's whole population and every local signal carried that re-ranking as a residual. **The tail operand (D-033)** follows the signal's `ripple` class in the grounding table: `max` for an *own* signal (a function of the file's own content and history — any movement is the instrument failing, and the own signals are the instrument's null check, expected to read 0.000), `p95` for a *coupled* signal (one whose value can change without the file being edited: an importer added, a test written, a manifest pointed at it — an edit elsewhere moving the value is the signal working, and a single node's flip is a fact about one edge). `max` is reported beside `p95` either way. **Floors (D-011):** the comparison is `untested` with reason `insufficient_stability_population` when fewer than `stability_min_n` (default 30) untouched nodes remain or more than `stability_max_excluded_frac` (default 0.5) of the common population was touched — the test must not get easier as the removed commits touch more of the repo. A file those commits edited has its recency, churn, and co-change legitimately move — that is the signal reporting the edit, not instability. The budget measures whether editing *other* files moves *this* file's value. Nodes born inside the removed window are absent from the perturbed run and so excluded by construction. `p95(|Δ|)` is reported beside `max`. A signal that swings past budget on a small edit is `untested` with reason `unstable` — never `asserted`. **Flags (D-029):** a G1 signal declared `flag` in the grounding table is a boolean that is never ranked; the degeneracy bar exists to keep a near-constant signal from being percentiled meaningfully, so it does not apply to a flag, while the stability budget does. The mapper refuses a percentile threshold over a flag.
...
| Class | What it covers | Correctness bar (beyond stability and non-degeneracy) | v0 members (D-011) |
|---|---|---|---|
| **G1 measurement** | direct git or file facts whose name is literal | none beyond stability + non-degeneracy. Where the instrument embeds a heuristic, the grounding table **declares it** (`heuristic`) and the report prints it: this is a *bounded, declared risk*, not a certification | `commit_count`, `churn_lines`, `fix_count`†, `revert_count`†, `author_count`†, `last_touched_days`, `recent_commit_share`, `size_loc`, `nesting_proxy`†, `cochange_degree`, `blame_age_median`†, `has_sibling_test`†, `is_package_entry`†‡, `is_config`†‡, `is_migration`†‡, `is_placeholder`†‡ (D-067), `is_example`†‡ (D-078) († = declared heuristic; ‡ = **flag**, D-029: a boolean that is never ranked — stability applies, the degeneracy bar does not, and a ruleset may read it only with an absolute term) |
| **G2 instrument-checked** | resolver-dependent facts, where the instrument can be wrong | τ-b vs an **independent second instrument measuring the same property**, bootstrap lower CI ≥ `tau_instrument` (default `0.60`). The counterpart must come from a different instrument (`METRIC_INSTRUMENT`, tested) and may not be one of the signal's own formula inputs | `fan_in`, `fan_out` ↔ `fan_in_alt` / `fan_out_alt`; `test_fan_in` ↔ `test_fan_in_alt` — all counterparts from the independent scanner (`repo-substrate-spec.md` §5) |
| **G3 cross-modal** | a **different modality** measures the same present-tense property | τ-b vs the counterpart, lower CI ≥ `tau_asserted` (default `0.30`). A G3 signal that is also a blend (`neglect_index`) carries `inputs` and must satisfy the G4 input rule too | `age_days`, `neglect_index` ↔ `blame_age_median` |
| **G4 derived** | fixed-weight blends and graph functions of asserted inputs | every input `asserted` (resolved first, in dependency order); the composite's **name carries no claim beyond its inputs** (D-004 Q3). The `inputs` list is tested against the actual weight keys so the two cannot drift | `fan_in_nonzero` (from `fan_in`); `centrality` (from `fan_in`); `load_index`; `complexity_proxy_index`; `reinforcement_index` (from `test_fan_in`) |

**Non-degeneracy (D-011, measured per D-014).** A signal whose modal value covers more than `degenerate_max_modal_share` (default `0.97`) of the population is `degenerate` on that repo and never `asserted`: a constant passes any stability budget trivially, which is Popper's objection made operational. The share, not a distinct-value count, is the measure: a binary flag with a real minority class is two-valued and not degenerate, while a constant (share 1.0) is. *(The first implementation counted distinct values and wrongly marked every binary signal degenerate; corrected the same day.)*

**Retirement criterion (D-011, D-014).** A G2/G3 pair whose bootstrap lower-CI τ is ≥ `tau_retire` (default `0.85`) on **every** reference repo cannot fail *on that set* and is flagged `non_discriminating`. It still counts as a falsifier **only if** a committed adversarial fixture proves the pair can fail on constructed input (`adversarial_fixture` in the grounding table; `retirement_backed` in `validation.json`; the report names the fixture or prints "NO FIXTURE"). History of the criterion on the reference set: before the `import type` fix, `fan_in` ↔ `fan_in_alt` ranged τ 0.37–0.98 (the 0.37 was typeorm — the instrument caught the instrument); after it, 0.98–1.00 on every repo, which is the expected post-fix state and trips the criterion. The fixture in `tests/test_instruments.py` makes the scanner over-count (an import-shaped string) and makes the primary instrument drop edges (`import type` with the flag off), so the pair is fixture-backed. `age_days` ↔ blame ranges 0.38–0.78 and is discriminating on its own.

**Reported correlates.** A signal may declare correlates that are computed and printed but never gate: `load_index` ~ `cochange_degree` and `test_fan_in`; `centrality` ~ `cochange_degree`; `neglect_index` ~ `cochange_degree`; `reinforcement_index` ~ `has_sibling_test`. On the first run `load_index` ~ co-change was τ 0.11–0.44 — informative (a foundation is imported by many and changed by few), not a correctness test.
...
- **Why commit-count, not wall-time.** An 80%-of-time split is distorted by idle gaps — a repo dormant for a year then active produces a near-empty or overstuffed holdout. An 80%-of-commits split keeps both windows populated on bursty histories. (A wall-time split is not implemented in v0; an earlier draft called it a config option, which it never was.) The split index is guarded: a repo too small to split (`n·(1−frac) < 1`) is refused, not silently split at HEAD.
...
> **A file is a positive iff it received ≥1 `fix`-type commit (`type ∈ {fix, revert}`) within the holdout window.**

No manual labeling, no judgment call. The label is a pure function of the commit subjects. **The classification is re-derived by the gate** from each holdout commit's subject with the §7 cascade and the validation config's own `label_subject_regex` (frozen in the validation fingerprint), **not** read from the substrate's `type` field, which the feature-side `fix_subject_regex` produced. The two regexes default to the same pattern, but narrowing the feature-side one — which shapes `fix_count` and therefore `bug_pressure_index` — cannot move a label (D-012; the "shared preimage" the circumvention pass found). The report prints both and flags a difference.
...
**Shape as emitted (schema `0.3`, D-011/D-012).** The example block below is illustrative of the *content*; the emitted shape differs from the June draft in three ways that the architecture validator flagged and that are now normative: (1) descriptive signals carry `grounding_class`, `counterpart`/`inputs`/`instrument`/`heuristic`, `non_discriminating`, and a `grounding.per_repo[]` list, each entry holding `stability` (with `n`, `distinct_values`, `n_excluded_touched`), `corroboration` (the τ block; the key is `corroboration`, not `cross_modal`), `correlates`, and `passed`/`reason`; `grounding.repos_passed` is a count; (2) predictive signals carry `holdout.per_repo[]` with `role`, per-repo metrics, `tau_vs_best_baseline`, `unavailable`, and the recognition comparison, plus `test_passes` / `test_ran` / `n_test_repos`; (3) the document carries `substrate_effective_config` (the fingerprint's preimage), `substrate_attestations` (seed + sha256 of every scored cache document), `tuned_config_commit`, and per-repo `role`, `expected_role`, and `recognition_seal`. The `reason` vocabulary is: `unstable`, `degenerate`, `corroboration_fail`, `too_few_pairs`, `insufficient_stability_population`, `input_not_asserted:<name>`, `no_stability_data`, `population_too_small`, `no_holdout_positives`, `no_holdout_negatives`, `insufficient_coverage`, `insufficient_signal`, `index_unavailable`, `no_test_repos`, `all_test_repos_degenerate`, `passed_on_N_of_M_test_repos_need_K`, `passed_on_N_repos_need_M`.
...
8. **Asserted-bar defaults (§2.4).** `stability_perturbation_k` (25 since D-031; 5 until then), `stability_eps` (0.01 since D-033; 0.05 until then), `stability_delta` (0.05 since D-033; 0.15 until then), `tau_asserted` (0.30), `M_asserted` (2), and `signal_floor_mult` (1.5) were placeholders accepted under D-004 Q4; D-033 placed eps and delta where they fire on the reference set once the re-ranking artefact was removed, and raised M_asserted to a majority.
