...
**Non-degeneracy (D-011).** A signal with fewer than `degenerate_min_distinct` (default 3) distinct values across the population is `degenerate` on that repo and never `asserted`: a constant passes any stability budget trivially, which is Popper's objection made operational.

**Retirement criterion (D-011).** A G2/G3 pair whose bootstrap lower-CI τ is ≥ `tau_retire` (default `0.85`) on **every** reference repo cannot fail and is therefore not a falsifier; it is flagged `non_discriminating` in `validation.json` and the report, and an adversarial fixture (alias-heavy, dynamic imports) is required before the pair counts again. On the 2026-09-04 run no pair tripped it: `fan_in` ↔ `fan_in_alt` ranged τ 0.37–0.98 across repos (the 0.37 was typeorm, where the primary extractor had dropped `import type` edges — the instrument caught the instrument), `age_days` ↔ blame 0.38–0.78.
