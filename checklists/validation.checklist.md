...
- [x] `[HOR]` A `kind: descriptive` signal reaches `asserted` only by passing **both** the stability budget (§2.4.1) **and** the cross-modal check (§2.4.2) on ≥ `M_asserted` repos (3 of 4 since D-033; `unit: test_predictive_can_never_be_asserted_and_descriptive_never_validated, test_g4_requires_every_input_asserted`); failing either yields `untested` with a specific reason (`unstable`, `cross_modal_fail`, `no_counterpart`), never a silent `asserted`. *(§2.4.4, §6.1)* `(unit)`
- [x] `[ALG]` Stability budget: substrate recomputed with the last K = 25 commits removed (D-031); percentiles re-ranked over the untouched population in both runs; pass iff `median(|Δ|) ≤ 0.05` and `tail(|Δ|) ≤ 0.15`, the tail being `max` for an *own* signal and `p95` for a *coupled* one (D-033); holdout-born nodes excluded. *(§2.4.1)* `(unit: test_stability_excludes_nodes_touched_by_removed_commits, test_local_signals_read_zero_after_reranking)`
...
- [x] `[GATE]` Degeneracy: a signal whose modal value covers more than `degenerate_max_modal_share` (0.97) of the population is `degenerate`, never `asserted` (D-011/D-014; a flag is exempt unless constant, D-030); stability floors (`n ≥ 30`, excluded ≤ 0.5) yield `insufficient_stability_population`. *(§2.4.1, §2.4.2)* `(unit)` — tests/test_gate.py
