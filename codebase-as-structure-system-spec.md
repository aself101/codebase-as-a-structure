...
C2 is optional in v0; it primarily enriches commit/incident signal and, later, extracts the *intended* structure from design docs for the blueprint overlay (§9). Everything else is on the v0 critical path.

Validation is not a pipeline stage — it is an offline gate that runs the substrate against itself over a temporal holdout and assigns each **signal** (composite index, percentile, or raw metric) a `validation_status`. The two predictive indices can be `validated` (forecast confirmed by holdout); present-tense descriptive signals are `asserted` (description confirmed by recognition/stability) — a different kind of grounding, not a lesser one. That status is what the anti-horoscope contract (§3) enforces at C3.
...
- **Phase 0 (v0).** C1 + a minimal C3 (in-repo percentile + a few topological predicates + the load/stress indices) + C5 brief via adversarial lens + C6 static cutaway. C2 optional, C4 light.
...
1. **C4/C6 boundary for the 2D cutaway.** For the cutaway, "massing" reduces nearly to vertical strata + feature placement; full geometric massing only matters for 3D. Decide whether C4 even exists as a distinct artifact in v0 or folds into C6.
...
3. **Mapping stability.** Quantify how much a small code change is allowed to move the skeleton, so the time-lapse stays continuous without hiding genuine structural shifts. This doubles as the **validation method for the non-predictive indices** (`load_index`, `neglect_index`, etc.) that the temporal holdout cannot falsify (`validation-spec.md` §2.1): an index that swings wildly on a one-line change is useless even if "correct."
