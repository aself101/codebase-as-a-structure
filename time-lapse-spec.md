...
The budget is pinned at K = 25 and applied from 25 to 50 (D-026, D-032; it was first pinned at K = 5, where D-024 showed it cannot fire). Between checkpoints K is typically tens to hundreds of commits, and the reading changes in two ways that the report must state rather than hide:
...
2. **A ceiling per K, and per geometry.** *Resolved D-026.* The budget is pinned at K = 25 and applied up to 50, by the rule that the unchanged ceiling (0.05) must sit between the reference set's per-transition median and its 90th percentile of jitter churn at the pinned K (`reports/2026-09-05-kstudy/kstudy.md`). No second ceiling per geometry: age-geometry floors are the least stable element of the skeleton (quantile bands re-rank wholesale under growth) and a looser ceiling would be tuned to pass; layer is the stable geometry for a time-lapse, and absolute-era bands for the age geometry are mapper §7 Q6.
